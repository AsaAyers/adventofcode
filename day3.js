...
const manhattanDistance = (a, b = origin) => Math.abs(a.x - b.x) + Math.abs(a.y - b.y)

function wireMap(str) {
  ...
  return str
    ...
    .map(line => {
      const dir = line[0]
      const num = Number(line.substr(1))
      ...
      for (var i = 0; i < num; i++) {
        ...
        tmp.distance = manhattanDistance(tmp)
...
function findIntersections(wire1, wire2, first = true) {
  const intersections = []
  const map1 = wireMap(wire1)
  const map2 = wireMap(wire2)

  let jStart = 0
  for (var i = 0; i < map1.length; i++) {
    const a = map1[i]
    for (var j = jStart; j < map2.length; j++) {
      const b = map2[j]
...
function findClosestIntersection(wire1, wire2) {
  return findIntersections(wire1, wire2)
    .reduce(
      (distance, [coord]) => Math.min(
        distance,
        manhattanDistance(origin, coord)
      ),
      Infinity
    )
}
...
function minimizeLatency(wire1, wire2) {
  return findIntersections(wire1, wire2, false)
    .reduce(
      (steps, [a, b]) => Math.min(steps, a.steps + b.steps),
      Infinity
    )
}
...
console.log('part2', minimizeLatency(input1, input2))
